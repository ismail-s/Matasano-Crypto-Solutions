import matasano_crypto_solutions.set1 as s


def test_task_1():
    res1 = s.hex_to_base64(
...
def test_task_2():
    x = s.hex_to_bytes('1c0111001f010100061a024b53535009181c')
    y = s.hex_to_bytes('686974207468652062756c6c277320657965')
    res2 = s.bytes_to_hex(s.xor(x, y))
    assert res2 == '746865206b696420646f6e277420706c6179'


def test_task_3():
    ciphertext = s.hex_to_bytes('1b37373331363f78151b7f2b783431333d78397828372'
                                'd363c78373e783a393b3736')
    res3 = s.decode_1_byte_xor(ciphertext)
    assert res3[1] == "Cooking MC's like a pound of bacon"


def test_task_4():
    ciphertexts = s.get_file('4.txt').split('\n')
    res4 = s.find_and_decrypt_ciphertexts(ciphertexts)
...
def test_task_5():
    ...
    res5 = s.bytes_to_hex(s.repeating_key_xor(s.text_to_bytes(plaintext5),
                                              s.text_to_bytes(key)))
...
def test_task_6():
    ...
    assert s.hamming_distance(string1, string2) == 37

    ciphertext6 = s.get_file('6.txt')
    ciphertext6 = s.base64_to_bytes(ciphertext6)
    res6 = s.decode_repeating_byte_xor(ciphertext6)
    assert res6[0] == 'Terminator X: Bring the noise'


def test_task_7():
    ciphertext7 = s.get_file('7.txt')
    ciphertext7 = s.base64_to_bytes(ciphertext7)
    password = b"YELLOW SUBMARINE"
    res7 = s.aes_ecb_decode(ciphertext7, password).decode('ascii')
    assert res7.startswith("I'm back and I'm ringin' the bell ")


def test_task_8():
    ciphertexts8 = s.get_file('8.txt').split('\n')
    ciphertexts8 = [bytes.fromhex(x) for x in ciphertexts8 if x]
    res8 = s.detect_aes_ecb_encrypted_texts(ciphertexts8)
    assert len(res8[1]) == 1
    most_likely_string = s.bytes_to_hex(res8[1][0])
