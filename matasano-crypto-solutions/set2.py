...
def encryption_oracle(plaintext: bytes) -> bytes:
    ...
    if (enc_func == aes_cbc_encode):
        print('oracle encrypted plaintext with cbc')
        return enc_func(to_encrypt, random_aes_key(), random_aes_key())
    else:
        print('oracle encrypted plaintext with aes')
        return enc_func(to_encrypt, random_aes_key())


def check_if_aes_or_cbc(text: bytes) -> str:
    counts = Counter(split_into_groups(ciphertext, 16))
...
print(check_if_aes_or_cbc(ciphertext))
